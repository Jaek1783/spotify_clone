...
useEffect(()=>{
...
},[isPlaying, songRef, volume])
  const handlePlay = () => {
    console.log(songUrl, songRef)
  ...
  const onPlaying = ()=>{
    if(songRef && songRef.current){
        ...
        setCurrentSong({"progress":Math.floor(currentTime / duration* 100)  , 'length': duration});
    }
  }
        ...
        ">
            <div 
                ...
                style={{
                    ...
                    // transform :`transitionX(${currentSong.progress})`+"%"
                }}
                ></div>
        </div>
        <audio src={songUrl} ref={songRef} onTimeUpdate={onPlaying}/>
